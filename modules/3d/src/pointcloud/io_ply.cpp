...
namespace cv {
    ...
    void PlyDecoder::readData(std::vector<Point3f> &points, std::vector<Point3f> &normals, std::vector<Point3f> &colors,
    ...
    void PlyEncoder::writeData(const std::vector<Point3f> &points, const std::vector<Point3f> &normals,
                               const std::vector<Point3f> &color, const std::vector<std::vector<int32_t>> &indices) {
        CV_UNUSED(indices);
        ...
        for (size_t i = 0; i < points.size(); i++) {
            ...
            if (hasColor) {
                file << (int)color[i].x << " " << (int)color[i].y << " " << (int)color[i].z;
            }
            if (isNormals) {
